...
# with full logging. Deployed to pc-deploy and run as a scheduled task.
#
# STEPS:
...
#
# LOGS: C:\deploy-setup.log        (appended, full transcript)
...
Run-Step 'Copy WIM files' {
    ...
    # Also check optical / virtual drives that already have install.wim
    ...
    foreach ($iso in $isoFiles) {
        ...
        if (Test-Path $src) {
            Write-Host "  WIM indexes in $src :"
            & dism /Get-WimInfo /WimFile:"$src"
            Write-Host "  Copying to $dest (may take several minutes)..."
            Copy-Item $src $dest -Force
            $sizeGB = [math]::Round((Get-Item $dest).Length / 1073741824, 1)
            Write-Host "  Copied $ver.wim ($sizeGB GB)." -ForegroundColor Green
            $copied += $ver
        } else {
            Write-Host "  WARN: No install.wim in mounted ISO $($iso.Name)" -ForegroundColor Yellow
        }
    ...
    foreach ($drv in $drives) {
        ...
        Write-Host "  Copying from $($drv.Root) to $dest ..."
        Copy-Item $src $dest -Force
    ...
    if ($copied.Count -eq 0) {
        ...
        Write-Host '  Copy WIM files manually:'
        ...
        Write-Host "  WIM files ready: $($copied -join ', ')" -ForegroundColor Green
    }
}
...
Write-Host "  Finished: $endTime  ($elapsed min)" -ForegroundColor Cyan
Write-Host ('=' * 60)
...
if ($allOk) {
    ...
    $summaryLines += 'RESULT: ONE OR MORE STEPS NEED ATTENTION'
    Write-Host '  ONE OR MORE STEPS NEED ATTENTION - see log for details.' -ForegroundColor Yellow
}

$summaryLines += ''
$summaryLines += 'REMAINING MANUAL STEPS:'
